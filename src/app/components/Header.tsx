  ...
  return(
    ...
    <div id="header" className='bg-white p-3 flex w-full justify-between items-center'>
      <div className='flex gap-4 items-center'>
        ...
        <p id="header-desktop" className={`cursor-pointer font-bold ${currentPage == "/" ? "text-[#50A98D]" : "text-[#CDCDCD]"}`} onClick={()=>handleCurrentPage("/")}>Home</p>
        <p id="header-desktop" className={`cursor-pointer font-bold ${currentPage == "/characters" ? "text-[#50A98D]" : "text-[#CDCDCD]"}`} onClick={()=>handleCurrentPage("/characters")}>Characters</p>
      </div>
      ...
      <Image id="header-mobile" alt="menu icon" onClick={()=>{setShowMenu(showMenu == "opacity-0" ? "opacity-100" : "opacity-0" )}} src="/menu.png" width={10} height={10} className="flex w-auto h-[25px] cursor-pointer"/>

      <div id="header-mobile" className={`absolute right-0 bg-slate-900 duration-200 top-14 ${showMenu}`}>
        <div className={`cursor-pointer font-bold p-2  ${currentPage == "/" ? "text-[#50A98D]" : "text-[#CDCDCD]"}`} onClick={()=>handleCurrentPage("/")}>Home</div>
        <div className={`cursor-pointer font-bold p-2 ${currentPage == "/characters" ? "text-[#50A98D]" : "text-[#CDCDCD]"}`} onClick={()=>handleCurrentPage("/characters")}>Characters</div>
        <div className="p-2 border-t-2 border-gray-700 cursor-pointer duration-200 hover:text-[#50A98D]" onClick={()=>{setViewModal("feedback")}}>Feedback</div>
        <div className="p-2 border-t-2 border-gray-700 bg-[#287B62] text-white py-2 px-3 cursor-pointer duration-200 hover:bg-[#50A98D]" onClick={()=>{currentUserId ? logOut() : setViewModal("login")}}>{currentUserId ? "Sign Out": "Sign In"}</div>
      </div>
